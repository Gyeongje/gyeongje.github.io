...
<div class="pf-projects">
  ...
  <div class="pf-project" id="hayyim">
    ...
    <p class="pf-project__desc"
       data-en="At HAYYIM SECURITY, vulnerability research and exploit development were performed across commercial software targets: MS Office, Web Browser, Antivirus, and RDP. Common workflow starts by mapping each program's internals and identifying user-input attack surfaces, then studying prior 1-day patch diffs to learn the bug-class taxonomy. Dynamic analysis via debugger plus static analysis on decompiled code surfaces size overflows, UAF, race conditions; for large codebases, open-source fuzzers are attached via custom harnesses for automated fuzzing combined with concurrent code auditing.<br><br><strong>Office (MS Office)</strong> — a DoS-class size-manipulation bug was found in the dynamic-linked Excel data parser via crafted OLE file extract data.<br><br><strong>Antivirus</strong> — during AV scanning of extracted OLE files, the decompression staged files into a globally-writable TEMP path; a Hard Link attack at that path lets system-privileged AV operations modify arbitrary files, enabling Local Privilege Escalation.<br><br><strong>Web Browser</strong> — renderer / IPC and other user-input surfaces analyzed with combined static / dynamic methods.<br><br><strong>RDP</strong> — RDP protocol input-handling paths analyzed for exploitable conditions."
       data-ko="하임시큐리티에서 MS Office, Web Browser, 백신(Antivirus), RDP 등 상용 소프트웨어를 대상으로 취약점 연구·익스플로잇 개발을 수행했습니다. 공통 분석 워크플로우는 우선 모든 소프트웨어의 상세 동작 과정을 파악하고, 사용자 입력을 받을 수 있는 attack surface를 조사하는 것에서 시작합니다. 이전 버전에서 발생한 1-day를 분석해 어떤 종류의 bug case가 발생하는지 함께 정리한 뒤, 디버거를 통해 사용자 입력을 주입하는 동적 분석과 디컴파일된 코드를 추적하는 정적 분석을 병행해 size Overflow, UAF, Race Condition 등 취약점을 도출합니다. 코드 규모가 큰 타깃에는 공개 Fuzzer를 활용해 취약 함수에 attach하는 Harness를 직접 작성하고, fuzzing 자동화와 코드 오디팅을 함께 진행했습니다.<br><br><strong>Office (MS Office)</strong> — Excel 데이터를 Parsing하는 Dynamic linking code 단을 연구 타깃으로 지정하였고, Excel OLE File Extract data를 변경해 size를 조작하는 DoS 단 취약점 케이스를 발견했습니다.<br><br><strong>Antivirus (백신)</strong> — 백신이 추출된 OLE File을 검사할 때, 압축 해제 과정에서 Full Access가 가능한 일반 TEMP 경로로 압축을 해제하는 취약점을 발견했습니다. 해당 경로에 Hard Link를 사용하면 시스템 권한이 경로 상의 파일을 직접 수정할 수 있게 되며, 이를 통해 Local Privilege Escalation으로 이어질 수 있음을 확인했습니다.<br><br><strong>Web Browser</strong> — 렌더러 / IPC 등 사용자 입력이 도달하는 핵심 surface를 대상으로 정적·동적 분석을 병행하며 취약점을 조사했습니다.<br><br><strong>RDP</strong> — RDP 프로토콜의 입력 처리 경로를 따라가며 취약점 분석과 익스플로잇 가능성 검증을 수행했습니다.">
